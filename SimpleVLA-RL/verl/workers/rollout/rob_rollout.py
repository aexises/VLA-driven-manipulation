...
def _worker_error_payload(stage: str, exc: Exception) -> dict:
    return {
        'type': 'error',
        'stage': stage,
...
def env_worker(task_name, task_id, trial_id, config, input_queue, output_queue, is_valid, global_steps, max_steps):
    ...
    try:
        ...
        output_queue.put(_worker_error_payload('env_init', exc))
    ...
    try:
        ...
        output_queue.put(_worker_error_payload('env_reset', exc))
        _close_libero_env(env)
        return
    ...
    while True:
        try:
            ...
            output_queue.put(_worker_error_payload('env_step', exc))
            _close_libero_env(env)
            return
        if action is None:
            _close_libero_env(env)
        ...
        for i in range(len(action)):
            try:
                ...
                if prev_action is None:
                    ...
                    smoothness_value = -float(np.square(np.asarray(a) - np.asarray(prev_action)).mean())
                prev_action = np.asarray(a, dtype=np.float32).copy()
                ...
                finish_step += 1
                progress_scores.append(reward_value)
                smoothness_scores.append(smoothness_value)
                terminal_scores.append(1.0 if done else 0.0)
                subgoal_scores.append(0.0)
...
class RobHFRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts):
        ...
        for item in output:
            for key, value in item.meta_info.get('metrics', {}).items():
    ...
    def _libero_mp_context(self):
        start_method = str(_config_get(self.config, 'multiprocessing_start_method', 'spawn'))
        return multiprocessing.get_context(start_method)

    def _start_libero_env_worker(self, mp_ctx, task_name, task_id, trial_id, is_valid, global_steps, max_steps):
        input_q = mp_ctx.Queue()
        output_q = mp_ctx.Queue()
        process = mp_ctx.Process(
            target=env_worker,
            args=(task_name, task_id, trial_id, self.config, input_q, output_q, is_valid, global_steps, max_steps),
        )
        process.daemon = True
        process.start()
        return process, input_q, output_q

    def _stop_libero_worker(self, process, input_queue=None, output_queue=None):
    ...
    def _recv_libero_worker_message(self, process, output_queue, timeout_s: float, stage: str):
        try:
            message = output_queue.get(timeout=timeout_s)
        except queue.Empty:
            if process.is_alive():
                return {'type': 'timeout', 'stage': stage}
            return {'type': 'crash', 'stage': stage, 'exitcode': process.exitcode}
        if isinstance(message, dict) and message.get('type') == 'error':
            return message
        return message

    def _record_rollout_issue(self, rollout_metrics: dict[str, float], event: dict) -> None:
        stage = str(event.get('stage', 'unknown'))
        event_type = str(event.get('type', 'error'))
        if stage in {'env_init', 'env_reset'}:
            rollout_metrics['rollout/env_init_failures'] += 1.0
        if stage == 'env_reset':
            rollout_metrics['rollout/env_reset_failures'] += 1.0
        if event_type == 'timeout':
            rollout_metrics['rollout/queue_timeout'] += 1.0
            if stage == 'env_init':
                rollout_metrics['rollout/env_init_timeout'] += 1.0
        elif event_type in {'crash', 'error'}:
            if stage in {'env_init', 'env_reset'}:
                rollout_metrics['rollout/env_init_crash'] += 1.0
            elif stage == 'env_step':
                rollout_metrics['rollout/env_step_crash'] += 1.0

    def _finalize_rollout_metrics(
        self,
        rollout_metrics: dict[str, float],
        env_init_times: list[float],
        env_step_times: list[float],
        worker_failures: float,
        batch_size: int,
    ) -> dict[str, float]:
        finalized = {key: float(value) for key, value in rollout_metrics.items()}
        finalized['rollout/env_init_time_mean'] = float(np.mean(env_init_times)) if env_init_times else 0.0
        finalized['rollout/env_step_time_mean'] = float(np.mean(env_step_times)) if env_step_times else 0.0
        finalized['rollout/worker_crash_rate'] = float(worker_failures / max(batch_size, 1))
        return finalized
    ...
    def _generate_minibatch_libero(self, prompts):
        ...
        mp_ctx = self._libero_mp_context()
        env_init_timeout_s = float(_config_get(self.config, 'env_init_timeout_s', 120))
        env_step_timeout_s = float(_config_get(self.config, 'env_step_timeout_s', 30))
        max_env_init_retries = int(_config_get(self.config, 'max_env_init_retries', 2))
        rollout_metrics = defaultdict(float)
        env_init_times = []
        env_step_times = []
        worker_failures = 0.0
        ...
        for idx in range(batch_size):
            ...
            input_q = None
            output_q = None
            for _ in range(max_env_init_retries + 1):
                process, input_q, output_q = self._start_libero_env_worker(
                    mp_ctx, task_name, t_id, tr_id, is_valid, global_steps, max_steps
                )
                init_start = time.time()
                init_data = self._recv_libero_worker_message(process, output_q, env_init_timeout_s, stage='env_init')
                env_init_times.append(time.time() - init_start)
                if isinstance(init_data, dict) and init_data.get('type') == 'init':
                    break
                worker_failures += 1.0
                last_error = init_data
                self._record_rollout_issue(rollout_metrics, init_data if isinstance(init_data, dict) else {'type': 'error', 'stage': 'env_init'})
                self._stop_libero_worker(process, input_q, output_q)
                process = None
                input_q = None
                output_q = None

            if process is None or init_data is None or init_data.get('type') != 'init':
                raise RuntimeError(f"Failed to initialize LIBERO worker for {task_name}:{t_id}:{tr_id}. Last error: {last_error}")

            processes.append(process)
            input_queues.append(input_q)
            output_queues.append(output_q)
            init_payloads.append(init_data)
        ...
        while step < max_steps:
            ...
            vla_output = self._generate_one_step(vla_input)
            actions = vla_output["action"]
            action_token_len = vla_output["responses"].shape[1] // self.config.action_chunks_len
            dense_progress_scores = torch.zeros_like(vla_output["responses"], dtype=torch.float32)
            dense_smoothness_scores = torch.zeros_like(vla_output["responses"], dtype=torch.float32)
            dense_terminal_scores = torch.zeros_like(vla_output["responses"], dtype=torch.float32)
            dense_subgoal_scores = torch.zeros_like(vla_output["responses"], dtype=torch.float32)
            ...
            step_data = {
                ...
                "pixel_values": vla_output["pixel_values"],
                "action": actions,
                "dense_progress_scores": dense_progress_scores,
                "dense_smoothness_scores": dense_smoothness_scores,
                "dense_terminal_scores": dense_terminal_scores,
                "dense_subgoal_scores": dense_subgoal_scores,
                "step": step
            }
            vla_history.append(step_data)
            ...
            for idx in active_indices:
                ...
                result = self._recv_libero_worker_message(processes[idx], output_queues[idx], env_step_timeout_s, stage='env_step')
                env_step_times.append(time.time() - step_start)
                if not isinstance(result, dict) or result.get('type') != 'step':
                    worker_failures += 1.0
                    failure_event = result if isinstance(result, dict) else {'type': 'error', 'stage': 'env_step'}
                    self._record_rollout_issue(rollout_metrics, failure_event)
                    task_records[idx]['active'] = False
                    task_records[idx]['complete'] = False
                    task_records[idx]['finish_step'] = max(
                        task_records[idx]['finish_step'],
                        step + self.config.action_chunks_len,
                    )
                    self._stop_libero_worker(processes[idx], input_queues[idx], output_queues[idx])
                    processes[idx] = None
                ...
                step_data["dense_progress_scores"][idx] = self._expand_action_scores(
                    result['progress_scores'], action_token_len, self.config.action_chunks_len, device=step_data["responses"].device
                )
                step_data["dense_smoothness_scores"][idx] = self._expand_action_scores(
                    result['smoothness_scores'], action_token_len, self.config.action_chunks_len, device=step_data["responses"].device
                )
                step_data["dense_terminal_scores"][idx] = self._expand_action_scores(
                    result['terminal_scores'], action_token_len, self.config.action_chunks_len, device=step_data["responses"].device
                )
                step_data["dense_subgoal_scores"][idx] = self._expand_action_scores(
                    result['subgoal_scores'], action_token_len, self.config.action_chunks_len, device=step_data["responses"].device
                )
                if is_valid:
        ...
        for process, input_q, output_q in zip(processes, input_queues, output_queues):
            if process is None:
                continue
            try:
                input_q.put(None)
        ...
        return self._prepare_output_batch(
            vla_history,
            task_records,
            batch_size,
            meta_info={
                'metrics': self._finalize_rollout_metrics(
                    rollout_metrics,
                    env_init_times=env_init_times,
    ...
    def _expand_action_scores(self, action_scores, action_token_len, action_chunks_len, device):
        flat_scores = []
        for score in action_scores:
            flat_scores.extend([float(score)] * action_token_len)
        pad_len = action_chunks_len * action_token_len - len(flat_scores)
        if pad_len > 0:
            flat_scores.extend([0.0] * pad_len)
        return torch.tensor(flat_scores, dtype=torch.float32, device=device)

    def _prepare_output_batch(self, vla_history, task_records, batch_size, meta_info=None):
        ...
        for optional_key in ["dense_progress_scores", "dense_smoothness_scores", "dense_terminal_scores", "dense_subgoal_scores"]:
            if optional_key in vla_history[0]:
                batch[optional_key] = []
                key_names.append(optional_key)
        ...
        return DataProto(batch=output_batch, meta_info=meta_info or {})
