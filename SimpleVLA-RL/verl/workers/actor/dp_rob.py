...
class RobDataParallelPPOActor(BasePPOActor):
    ...
    def __init__(
        ...
        self.use_proprio = bool(_config_get(self.config, 'use_proprio', False))
    ...
    def update_policy(self, data: DataProto):
        ...
        for batch_idx, data in enumerate(dataloader):
            ...
            for test_idx, data in enumerate(micro_batches):
                ...
                ref_log_prob = data['ref_log_prob'] if 'ref_log_prob' in data.keys() else None
                ...
                for i in range(0, traj_len, int(traj_len/traj_split_num)):
                    ...
                    ref_log_prob_tmp = ref_log_prob[:, slice_id: next_slice_id] if ref_log_prob is not None else None
                    ...
                    valid_positions = response_mask_tmp.bool()
                    if valid_positions.any():
                        valid_old = old_log_prob_tmp[valid_positions].detach()
                        valid_new = log_prob[valid_positions].detach()
                        valid_ratio = torch.exp(valid_new - valid_old)
                        old_logprob_values.append(valid_old)
                        new_logprob_values.append(valid_new)
                        ratio_values.append(valid_ratio)
                        ratio_clip_low += int((valid_ratio < (1.0 - clip_ratio_low)).sum().item())
                        ratio_clip_high += int((valid_ratio > (1.0 + clip_ratio_high)).sum().item())
                        ratio_count += int(valid_ratio.numel())
                        if ref_log_prob_tmp is not None:
                            ref_logprob_values.append(ref_log_prob_tmp[valid_positions].detach())

                if ratio_count > 0:
                    all_old = torch.cat(old_logprob_values, dim=0)
                    all_new = torch.cat(new_logprob_values, dim=0)
                    all_ratio = torch.cat(ratio_values, dim=0)
                    loss_info['actor/logprob_old_mean'] = all_old.mean().item()
                    loss_info['actor/logprob_new_mean'] = all_new.mean().item()
                    loss_info['actor/ratio_mean'] = all_ratio.mean().item()
                    loss_info['actor/ratio_std'] = all_ratio.std(unbiased=False).item()
                    loss_info['actor/ratio_clip_low_frac'] = ratio_clip_low / ratio_count
                    loss_info['actor/ratio_clip_high_frac'] = ratio_clip_high / ratio_count
                    if ref_logprob_values:
                        loss_info['actor/logprob_ref_mean'] = torch.cat(ref_logprob_values, dim=0).mean().item()
