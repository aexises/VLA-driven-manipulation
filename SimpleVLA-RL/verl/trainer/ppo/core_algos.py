...
def compute_grpo_dense_advantage(
    ...
    """
    Compute dense-return GRPO advantages using token-level discounted returns.

    Args:
        token_level_rewards: `(torch.Tensor)` shape: (bs, response_length)
        eos_mask: `(torch.Tensor)` shape: (bs, response_length)
        index: prompt uid for grouping
        gamma: discounted return factor
    """
    with torch.no_grad():
        rewards = token_level_rewards * eos_mask
        returns = torch.zeros_like(rewards)
        for step in reversed(range(rewards.shape[1])):
            if step == rewards.shape[1] - 1:
                returns[:, step] = rewards[:, step]
            else:
                returns[:, step] = rewards[:, step] + gamma * returns[:, step + 1]
            returns[:, step] = returns[:, step] * eos_mask[:, step]

        normalized_returns = torch.zeros_like(returns)
        id2positions = defaultdict(list)
        batch_size = returns.shape[0]
        for row_index in range(batch_size):
            id2positions[index[row_index]].append(row_index)

        for uid, positions in id2positions.items():
            valid_values = []
            for pos in positions:
                valid_values.append(returns[pos][eos_mask[pos].bool()])
            if len(valid_values) == 0:
                continue
            flat_values = torch.cat(valid_values, dim=0) if valid_values else torch.zeros(1, device=returns.device)
            mean = flat_values.mean()
            std = flat_values.std(unbiased=False)
            safe_std = std if std > epsilon else torch.tensor(1.0, device=returns.device)
            for pos in positions:
                normalized_returns[pos][eos_mask[pos].bool()] = (
                    returns[pos][eos_mask[pos].bool()] - mean
                ) / safe_std

        advantages = torch.zeros_like(normalized_returns)
        for step in reversed(range(normalized_returns.shape[1])):
            if step == normalized_returns.shape[1] - 1:
                advantages[:, step] = normalized_returns[:, step]
            else:
                if discounted_advantage:
                    advantages[:, step] = normalized_returns[:, step] + gamma * advantages[:, step + 1]
                else:
                    advantages[:, step] = normalized_returns[:, step] + advantages[:, step + 1]
            advantages[:, step] = advantages[:, step] * eos_mask[:, step]
