...
from verl.trainer.ppo.rob_reward import (
    ...
    build_binary_reward_tensor,
    combine_dense_rewards,
...
class RobRewardManager():
    ...
    def __call__(self, data: DataProto):
        ...
        complete = data.batch['complete']
        verifier_reward = build_binary_reward_tensor(
            complete=complete,
        ...
        if self.reward_spec.reward_type == "binary":
            valid_mask = build_valid_response_mask(
                data.batch['finish_step'],
                reward_tensor.shape[1],
            ...
            reward_metrics['reward_all'] = (reward_tensor * valid_mask.to(dtype=reward_tensor.dtype)).sum(dim=-1).mean(dim=0).item()
            reward_metrics['reward_mean'] = reward_tensor.mean().item()
            reward_metrics['reward_mean_padded'] = reward_tensor.mean().item()
            valid_values = reward_tensor[valid_mask]
            reward_metrics['reward_mean_valid'] = valid_values.mean().item() if valid_values.numel() > 0 else 0.0
            reward_metrics['reward_sum_sequence_mean'] = reward_metrics['reward_all']
            reward_metrics['negative_ratio'] = float((reward_tensor < 0).float().mean().item())
            reward_metrics['negative_ratio_valid'] = float((valid_values < 0).float().mean().item()) if valid_values.numel() > 0 else 0.0
            reward_metrics['clip_activation_ratio'] = 0.0
            reward_metrics['clip_activation_ratio_valid'] = 0.0
            reward_metrics['clip_active'] = 0.0
            reward_metrics['valid_token_count_mean'] = float(valid_mask.sum(dim=-1).float().mean().item())
            reward_metrics['finish_step_mean'] = float(data.batch['finish_step'].float().mean().item())
        else:
            reward_tensor_dict, dense_metrics = combine_dense_rewards(data.batch, self.reward_spec)
            reward_metrics.update(dense_metrics)
            reward_tensor_dict['gt_scores'] = verifier_reward
